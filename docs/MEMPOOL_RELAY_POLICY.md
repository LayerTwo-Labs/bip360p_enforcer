...
The end-to-end wallet lifecycle is exercised by the `bip360_wallet_lifecycle`
integration trial (`Mode::GetBlockTemplate`).

## Signing and relay workflow

1. Build a P2MR output and signed spend with the enforcer wallet
   (`CreateP2mrAddress` → `SpendP2mr`, backed by `pqc::signer`).
2. The nonstandard spend is injected into the enforcer's own block template and
   mined via `submitblock` — stock Core will not relay PQC-sized witnesses via
   `sendrawtransaction`.
3. `connect_block` / `accept_tx` enforce the overload rules on the mined block.
