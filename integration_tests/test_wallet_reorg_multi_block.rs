...
use crate::{
    ...
    setup::{Mode, Network, PostSetup, PreSetup, SetupOpts},
    util::BinPaths,
};
...
pub async fn test_wallet_reorg_multi_block(bin_paths: BinPaths) -> anyhow::Result<()> {
    ...
    // Upstream's enforcer also runs a rejected-block scenario here, which
    // requires consensus rules that can reject a block. The template's empty
    // rule set never rejects blocks, so that scenario has no equivalent
    // until a fork adds rules.
