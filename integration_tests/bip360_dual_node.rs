...
pub fn bip360_p2p_setup_opts(peer_listen_port: u16, bitcoind_kind: BitcoindKind) -> SetupOpts {
    SetupOpts {
        ...
        enforcer_wallet: crate::setup::EnforcerWallet::Enabled,
        bitcoind_args: vec![
...
/// - Both `Mode::NoMempool` + `EnforcerWallet::Disabled` (no electrs; `setup-core` enough).
...
pub async fn broadcast_to_peer(
    ...
    let peer_addr: bip300301_enforcer_lib::p2p::BroadcastAddr =
        format!("127.0.0.1:{peer_listen_port}")
            .parse()
            .map_err(|err| anyhow::anyhow!("invalid peer P2P address: {err}"))?;

    for attempt in 0..3 {
        match broadcast_nonstandard_tx(peer_addr.clone(), height, magic, tx.clone()).await {
