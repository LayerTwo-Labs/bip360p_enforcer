...
echo "=== bip360p_enforcer Performance Analysis ==="
echo
...
if [ ! -f "./target/release/bip360p_enforcer" ]; then
    echo "   ERROR: Binary not found. Building first..."
    cargo build --release
fi

./target/release/bip360p_enforcer --data-dir ./datadir --node-rpc-addr=localhost:38332 --node-rpc-user=user --node-rpc-pass=password --enable-mempool > trace_results/enforcer.log 2>&1 &
ENFORCER_PID=$!
...
ACTUAL_PID=$(pgrep -f "bip360p_enforcer.*--data-dir")
