...
///
/// Comfortably accommodates the heaviest supported leaf, hybrid EC+SLH (a 64-byte
/// Schnorr signature plus a ~7856-byte SLH-DSA signature); the extra headroom
/// leaves room for future multi-signature leaves without a consensus change.
...
mod tests {
    ...
    #[test]
    fn hybrid_ec_slh_sig_fits_per_input_wu_budget() {
        let total =
            estimate_sig_wu(SCHNORR_SIGNATURE_SIZE) + estimate_sig_wu(SLH_DSA_128S_SIGNATURE_SIZE);
        assert_eq!(total, 7_920);
