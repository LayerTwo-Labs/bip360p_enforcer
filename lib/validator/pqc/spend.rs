...
pub enum SpendError {
    ...
    #[error(
        "cannot verify P2MR spend at input {input_index}: a co-input prevout required by the \
         committing sighash is unavailable"
    )]
    UnverifiableSpend { input_index: usize },
    #[error(transparent)]
...
pub fn validate_transaction_spends(
    ...
    for (input_index, input) in tx.input.iter().enumerate() {
        ...
        let Some(prevouts) = prevouts_for_p2mr_input(
            ...
            // We have established this input spends a P2MR output with a
            // non-empty witness, so it MUST be verified. A non-ANYONECANPAY
            // sighash commits to every input's prevout; if a co-input's prevout
            // is unavailable we cannot reconstruct the sighash, so fail closed —
            // skipping here would let an attacker bypass the signature check by
            // pairing a victim P2MR input with any untracked (non-P2MR) co-input.
            return Err(SpendError::UnverifiableSpend { input_index });
        };
...
mod tests {
    ...
    /// A P2MR input paired with an untracked (non-P2MR) co-input under a
    /// committing (non-ANYONECANPAY) sighash must be REJECTED, not skipped.
    /// Otherwise an attacker bypasses the signature check entirely by pairing a
    /// victim P2MR input with any co-input whose prevout the enforcer does not
    /// track. Regression test for the multi-input validation-skip bypass.
    #[test]
    fn validate_transaction_spends_rejects_p2mr_with_untracked_non_acp_coinput() {
        let secp = Secp256k1::new();
        let keypair = Keypair::new(&secp, &mut thread_rng());
        let ec_pk: [u8; 32] = XOnlyPublicKey::from_keypair(&keypair).0.serialize();
        let leaf_script = build_push32_checksig_leaf(&ec_pk);
        ...
        // Any witness — with the fix, the spend is rejected before it is checked.
        let mut witness = Witness::new();
        witness.push(append_sighash(vec![0u8; 64], TapSighashType::All));
        witness.push(leaf_script.as_bytes());
        witness.push(single_leaf_control_block());

        // input 0: victim P2MR UTXO (tracked). input 1: attacker's untracked co-input.
        let tx = Transaction {
            version: bitcoin::transaction::Version::TWO,
            lock_time: bitcoin::locktime::absolute::LockTime::ZERO,
            input: vec![
        ...
        // Only the P2MR input's prevout is tracked; the co-input's is not.
        let mut available_prevouts = HashMap::new();
        available_prevouts.insert(funding_outpoint, prevout);

        let err = validate_transaction_spends(&tx, &available_prevouts, None, None, &mut None)
            .expect_err("multi-input P2MR spend with an untracked co-input must be rejected");
        assert!(
            matches!(err, SpendError::UnverifiableSpend { input_index: 0 }),
            "expected UnverifiableSpend at input 0, got {err:?}"
        );
    }
    ...
    fn p2mr_signer_roundtrip_schnorr() {
        ...
        use super::super::signer::{SignAlgorithm, sign_p2mr_script_path_spend};
